...
/// Computes the swap amounts and updates the Whirlpool and Tick sequence state.
///
/// # Arguments
/// - `token_amount`: Input or output amount for swap (must be non-zero)
/// - `sqrt_price_limit`: Square root price limit, 0 means use min/max
/// - `whirlpool`: Current Whirlpool state
/// - `tick_sequence`: Tick sequence for price levels
/// - `a_to_b`: true = A->B, false = B->A
/// - `specified_input`: true = token_amount is input, false = token_amount is output
/// - `timestamp`: used for adaptive fee calculation
/// - `adaptive_fee_info`: optional adaptive fee info
///
/// # Returns
...
/// Swap 计算，同时返回 whirlpool 和 tick_sequence 副本状态
...
#[allow(clippy::too_many_arguments)]
pub fn compute_swap_with_state<const SIZE: usize>(
    token_amount: u64,
    sqrt_price_limit: u128,
    whirlpool: WhirlpoolFacade,
    tick_sequence: TickArraySequence<SIZE>,
    ...
    update_whirlpool.sqrt_price = current_sqrt_price;
    update_whirlpool.tick_current_index = current_tick_index;
    update_whirlpool.liquidity = current_liquidity;
    Ok(SwapResultWithState {
        token_a,
        token_b,
        trade_fee,
        applied_fee_rate_min: applied_fee_rate_min.unwrap_or(base_fee_rate as u32),
        applied_fee_rate_max: applied_fee_rate_max.unwrap_or(base_fee_rate as u32),
        whirlpool: update_whirlpool,
    })
}
// Private functions
